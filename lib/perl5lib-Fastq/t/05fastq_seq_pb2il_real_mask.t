...
my $parser = new_ok('Fastq::Parser', ['file' => $RealBin.'/05pacbio_masked.fq']);
can_ok($parser, 'next_seq');
my $pacbio_seq = $parser->next_seq();
isa_ok($pacbio_seq, $Class);
can_ok($pacbio_seq, 'pb2il');
my @pb2il_reads = $pacbio_seq->pb2il('paired' => 1, 'length' => 100, 'insert' => 200, 'step' => 50, 'masked' => 1);
...
my $parser_il1 = new_ok('Fastq::Parser', ['file' => $RealBin.'/05illumina_1.fq']);
my $parser_il2 = new_ok('Fastq::Parser', ['file' => $RealBin.'/05illumina_2.fq']);

subtest 'pb2il - paired length=100 inser=200 step=50 masked' => sub{
	my $i=0;
...
my @pb2il_unpaired = $pacbio_seq->pb2il('paired' => 0, 'length' => 100, 'insert' => 200, 'step' => 50, 'masked' => 1);
my $parser_il = new_ok('Fastq::Parser', ['file' => $RealBin.'/05illumina_1.fq']);
subtest 'pb2il - unpaired length=100 inser=200 step=50 masked' => sub{
	my $i=0;
